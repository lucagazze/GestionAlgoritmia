...
export default function ClientPortalPage() {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-zinc-900"><Loader2 className="w-8 h-8 animate-spin text-zinc-400" /></div>;
    ...
    return (
        <div className="min-h-screen bg-[#FAFAFA] dark:bg-zinc-950 font-sans text-zinc-900 dark:text-white selection:bg-black selection:text-white">
            
            {/* Header */}
            <div className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-20">
                <div className="max-w-5xl mx-auto px-6 py-4 flex justify-between items-center">
                    <div>
                        <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1">Portal de Cliente</p>
            ...
            <div className="max-w-5xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <section>
                        ...
                        <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                            <div className="h-4 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden mb-4">
                                <div 
                    ...
                    <section>
                        ...
                        {deliverables.length === 0 ? (
                            <div className="bg-zinc-50 dark:bg-zinc-900/50 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl p-8 text-center text-zinc-400">
                                <p>No hay entregas pendientes de revisión por el momento.</p>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                {deliverables.map(deliv => (
                                    <div key={deliv.id} className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm transition-all hover:shadow-md">
                                        ...
                                        {deliv.status === 'PENDING' && (
                                            <div className="flex gap-3 mt-6 border-t border-zinc-100 dark:border-zinc-800 pt-4">
                ...
                <div className="space-y-8">
                    ...
                    {/* Chat Widget */}
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm h-[500px] flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-zinc-50/50 dark:bg-zinc-950" ref={chatRef}>
                            {messages.length === 0 && <p className="text-center text-xs text-zinc-400 mt-10">Envía un mensaje para comenzar...</p>}
                            {messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.sender === 'CLIENT' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[80%] p-3 rounded-xl text-sm ${msg.sender === 'CLIENT' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-white border border-zinc-200 text-zinc-800 rounded-bl-none'}`}>
                                        <p>{msg.content}</p>
                                        <div className={`text-[10px] mt-1 text-right ${msg.sender === 'CLIENT' ? 'text-blue-200' : 'text-zinc-400'}`}>
                    ...
                    {/* Resources Mini */}
                    <section>
                        <h2 className="text-sm font-bold uppercase tracking-widest text-zinc-400 mb-3">Recursos Compartidos</h2>
                        <div className="space-y-2">
                            {project.resources?.map((res, idx) => (
                                <a key={idx} href={res.url} target="_blank" className="block p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl hover:border-blue-300 transition-colors">
                                    <div className="flex items-center gap-3">
                            ...
                            {(!project.resources || project.resources.length === 0) && <p className="text-xs text-zinc-400 italic">No hay recursos.</p>}
                        </div>
                    </section>
                </div>
            </div>
            ...
            <Modal isOpen={isFeedbackModalOpen} onClose={() => setIsFeedbackModalOpen(false)} title="Solicitar Cambios">
                <div className="space-y-4">
                    <p className="text-sm text-zinc-600">Describe qué cambios necesitas para el entregable <strong>{selectedDeliv?.name}</strong>:</p>

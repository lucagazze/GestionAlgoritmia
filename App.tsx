...
import { Calculator, LayoutGrid, CheckSquare, Briefcase, Menu, X, ArrowUpRight } from 'lucide-react';
...
const Sidebar = ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) => {
  ...
  const links = [
    { path: '/', icon: <Calculator className="w-5 h-5" />, label: 'Calculadora' },
    { path: '/projects', icon: <Briefcase className="w-5 h-5" />, label: 'Proyectos & MRR' },
      ...
      `}>
        <div className="h-20 flex items-center px-8 border-b border-gray-100">
          ...
          <div>
            ...
            <span className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Agency OS 2.0</span>
          </div>
        ...
        <nav className="p-6 space-y-2 flex-1">
          ...
          <div className="pt-8 mt-4 border-t border-gray-200/50">
             <p className="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Acceso Rápido</p>
             <a href="https://algoritmiadesarrollos.com.ar/" target="_blank" rel="noreferrer" className="flex items-center gap-3 px-4 py-2 text-sm text-gray-500 hover:text-black transition-colors">
                <ArrowUpRight className="w-4 h-4" /> Ir a Sitio Web
             </a>
          </div>
        </nav>
